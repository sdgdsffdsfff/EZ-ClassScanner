package com.ecfront.easybi.classscanner.inner;
...
public class ClassScanExecutor {
    ...
    public Set<Class<?>> execute() throws IOException, ClassNotFoundException {
        ...
        while (urls.hasMoreElements()) {
            ...
            if ("file".equals(protocol)) {
                ...
                findAndAddClassesByJar(((JarURLConnection) url.openConnection())
                        .getJarFile(), packageDir);
            }
        }
        return results;
    }

    private void findAndAddClassesByFile(String currentPackage,
                                         File currentFile) throws ClassNotFoundException {
    ...
    private void findAndAddClassesByJar(
            JarFile jar, String currentPath) throws ClassNotFoundException {
            ...
            if (jarName.startsWith(currentPath)) {
                ...
                if (jarName.endsWith(".class")
                    ...
                    String className = jarName.substring(jarName.lastIndexOf('/')+1,
                            jarName.length() - 6);
                    Class<?> clazz = Class.forName(jarName.substring(0, idx)
                            .replace('/', '.')
                            + '.' + className);
                    if (mather.isMath(clazz)) {
                        results.add(clazz);
